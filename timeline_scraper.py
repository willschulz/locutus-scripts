...
def preprocess_timelines(t):
  retain_columns = ['id', 'created_at', 'in_reply_to_id', 'in_reply_to_account_id', 'visibility', 'language', 'uri', 'url', 'replies_count',
                      'reblogs_count', 'favourites_count', 'edited_at', 'content', 'reblog', 'application', 'media_type', 'media_url', 'account_id', 'account_username', 'account_acct', 'scraped_at']
...
query = f"SHOW TABLES LIKE 'timelines'"
cursor.execute(query)
table_exists = cursor.fetchone() is not None
cursor.close()

if table_exists:
    if pd.read_sql_query('SELECT COUNT(id) FROM timelines', dbconn)['COUNT(id)'][0] > 0:
        scrape_type = 'update'
    else:
        scrape_type = 'init'
else:
...
if scrape_type == 'update':
    since_id_from_db = pd.read_sql_query('SELECT id FROM timelines ORDER BY id DESC', dbconn)['id'][0:40].tolist()
    print("Updating timelines back to since_id: " + str(max(since_id_from_db)))
