# updating like scraper... problem with "Duplicate entry '109667507851426797_110710632286206263' for key 'likes.PRIMARY'"
...
query = f"SHOW TABLES LIKE 'likes'"
cursor.execute(query)
table_exists = cursor.fetchone() is not None
cursor.close()

if table_exists:
    if pd.read_sql_query('SELECT COUNT(id) FROM likes', dbconn)['COUNT(id)'][0] > 0:
        scrape_type = 'update'
    else:
        scrape_type = 'init'
else:
...
#     already_scraped_favorite_ids = pd.read_sql_query('SELECT id FROM likes ORDER BY id DESC', con)['id'][0:40].tolist()
#     con.close()
#     print("Updating likes back to since_id: " + max(since_id_from_db))
#####

# get ids to check
now_timestamp=datetime.now().timestamp()
ids_to_check = pd.read_sql_query('SELECT id FROM timelines WHERE created_at > ' + str(now_timestamp-60*60*24*days_back), dbconn).id.to_list()
...
while len(statuses_remaining) > 0:
    ...
    if fdf.shape[0]>0:
            ...
            if table_exists:
                ...
                previously_scraped_likers = pd.read_sql_query('SELECT acct FROM likes WHERE liked_status_id = ' + str(this_status_id), dbconn)
            ...
            if fdf_new.shape[0]>0:
              ...
              for _, row in fdf_new.iterrows():
                insert_query = """
                    INSERT INTO likes (id, acct, liked_status_id, scraped_at, lid)
                    VALUES (%s, %s, %s, %s, %s)
                """
                values = tuple(row)
