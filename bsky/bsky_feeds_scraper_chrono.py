...
create_table_query = """
    CREATE TABLE IF NOT EXISTS bsky_posts_feedbased_chrono (
        post_uri TINYTEXT,
        post_cid TINYTEXT,
...
for index, row in bsky_feeds.iterrows():
    ...
    if len(df) > 0:
        ...
        existing_posts = pd.read_sql_query("SELECT post_cid FROM bsky_posts_feedbased_chrono", dbconn)["post_cid"].values
        ...
        if len(df) > 0:
            try:
              ...
              insert_query = f"INSERT INTO bsky_posts_feedbased_chrono ({', '.join(df.columns)}) VALUES ({', '.join(['%s'] * len(df.columns))})"
